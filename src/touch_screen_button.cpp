...
void TouchScreenButton::_notification(int p_what) {
	switch (p_what) {
		...
		case NOTIFICATION_EXIT_TREE: {
			if (is_pressed()) {
				_release(Ref<InputEventScreenTouch>(), true);
			}
		} break;
		case NOTIFICATION_VISIBILITY_CHANGED: {
			...
			if (is_visible_in_tree()) {
				...
				if (is_pressed()) {
					_release(Ref<InputEventScreenTouch>(), false);
				}
			}
		} break;
...
void TouchScreenButton::_input(const Ref<InputEvent> &p_event) {
	...
	if (passby_press) {
		...
		if (st && !st->is_pressed() && finger_pressed == st->get_index()) {
			_release(Ref<InputEventScreenTouch>(st), false);
		}
...
void TouchScreenButton::_release(const Ref<InputEventScreenTouch> &p_touch_event, bool p_exiting_tree) {
	...
	if (!p_exiting_tree) {
		if (!p_touch_event.is_null()) {
			was_inside = _is_point_inside(p_touch_event->get_position());
		}
		emit_signal("released", was_inside);
		update();
	}
}
...
void TouchScreenButton::_bind_methods() {
	...
	ADD_SIGNAL(MethodInfo("released", PropertyInfo(Variant::BOOL, "was_inside")));
